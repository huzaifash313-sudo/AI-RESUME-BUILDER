...
const MinimalTemplate = ({ data, accentColor }) => {
    // KHRABI FIX: Date formatting ko safe kiya taake invalid strings par UI crash na ho
    ...
    return (
        <div className="max-w-4xl mx-auto p-12 bg-white text-slate-900 font-sans min-h-[29.7cm] shadow-sm">
            ...
            <header className="mb-12 border-b pb-8 border-slate-100">
                <h1 className="text-5xl font-extralight mb-4 tracking-tighter text-slate-800">
                    ...
                    <div className="col-span-3 space-y-10">
                        {data.experience.map((exp, index) => (
                            <div key={index} className="group">
                                ...
                                {exp.description && (
                                    <div className="text-slate-600 leading-relaxed font-light text-sm border-l border-slate-100 pl-4">
                                        {exp.description}
                                    </div>
                                )}
                            </div>
                ...
                <section className="mb-12 grid grid-cols-4 gap-4">
                    ...
                    <div className="col-span-3 grid grid-cols-1 md:grid-cols-2 gap-8">
                        {data.project.map((proj, index) => (
                            <div key={index}>
                                <h3 className="text-base font-semibold mb-2 text-slate-800 underline decoration-slate-200 underline-offset-4">{proj.name}</h3>
