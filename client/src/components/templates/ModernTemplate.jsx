                ...
                {data.professional_summary && (
                    <section className="mb-12">
                        <div className="flex items-center gap-4 mb-4">
                            <h2 className="text-xs font-black uppercase tracking-[0.3em] whitespace-nowrap text-slate-400">
                            ...
                            {data.experience.map((exp, index) => (
                                <div key={index} className="group relative">
                                    <div className="flex flex-col md:flex-row justify-between items-baseline mb-2">
                                        <h3 className="text-xl font-bold text-slate-900 group-hover:text-blue-600 transition-colors uppercase tracking-tight">{exp.position}</h3>
                                    ...
                                    <p className="text-sm font-bold mb-4 uppercase tracking-widest" style={{ color: accentColor }}>{exp.company}</p>
                                    {exp.description && (
                    ...
                    <section className="mb-12">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {data.project.map((p, index) => (
                                <div key={index} className="p-6 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                                    <h3 className="text-lg font-bold text-slate-800 mb-2 uppercase tracking-tight">{p.name}</h3>
                        ...
                        <section>
                            ...
                            <div className="space-y-6">
                                {data.education.map((edu, index) => (
                                    <div key={index}>
                                        <h3 className="font-bold text-slate-900 text-base leading-tight uppercase">
                                            {edu.degree}
                                        </h3>
                                        <p className="text-sm font-medium mt-1" style={{ color: accentColor }}>{edu.institution}</p>
                                        <div className="flex justify-between items-center text-[10px] font-bold text-slate-400 uppercase mt-2 tracking-widest">
                        ...
                        <section>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {data.skills.map((skill, index) => (
                                    <span
                                        key={index}
                                        className="px-4 py-1.5 text-[11px] font-bold rounded-lg transition-all border uppercase tracking-wider"
                                        style={{ 
